...
- **Status**: DONE — step 1 (MODULARIZATION_PLAN) session-28; step 2 (MODULE_CONTRACTS_DRAFT) **session-29**. Both steps sealed; roll-up host resolved (objectives-hosted `rollup.*` read contract). NotificationPort §7 owner still flagged for founder ratification (carried on the plan + the draft revisit triggers).
- **Size**: M (2 ticks)
...
- **Definition of done**:
  ...
  - ✓ MODULE_CONTRACTS_DRAFT.md with TS interface signatures — **done session-29**; all 13 USER_JOURNEYS J-index rows traced to signatures; route sets cite API_SURFACE; edges cite MODULE_DEPENDENCY_GRAPH; 8 blocks + AIR provider + 2 shells + NotificationPort + Layer-4 orchestrator + PageContract (stage-weather/rulesSurface); contract-test shape per block (no impls, rule 7)
