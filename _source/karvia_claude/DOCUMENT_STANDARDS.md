...
<!-- @GENOME T2-GOV-002 | ACTIVE | 2026-03-30 | parent:T0-GOV-001 | auto:/strategy | linked:/audit -->
...
|---------|------|--------|---------|
| 2.0.0 | Jan 7, 2026 | BRAMHI_LABS_CONSULTANT | Added Section 10.4 Link Verification, Section 15 Link Validation |
...
6. VERIFY ALL LINKS → Use Glob tool to confirm paths exist (Section 10.4)
```
...
### 10.4 Link Verification (MANDATORY)

**CRITICAL**: Before creating any document that contains links to other files:

```
LINK VERIFICATION CHECKLIST:
[ ] For EACH link in the document:
    [ ] Verify the target file EXISTS (use Glob or ls command)
    [ ] Verify the PATH is correct (including subfolder structure)
    [ ] If file doesn't exist yet, mark link as [TODO] or [PLANNED]
[ ] For INDEX files (00_MASTER_INDEX.md, README.md):
    [ ] Verify EVERY listed document exists
    [ ] Run link audit before finalizing
[ ] Never reference files that don't exist without marking them
```

**Common Path Errors to Avoid**:
```
WRONG: ./1-PRODUCT/strategy/ENGINE_SCORING_STRATEGY.md
RIGHT: ./1-PRODUCT/strategy/03-engines/ENGINE_SCORING_STRATEGY.md

WRONG: ./1-PRODUCT/strategy/MVG_MINIMUM_VIABLE_GOVERNANCE.md
RIGHT: ./1-PRODUCT/strategy/00-constitutional/MVG_MINIMUM_VIABLE_GOVERNANCE.md
```

**Strategy Folder Structure Reference**:
```
1-PRODUCT/strategy/
├── 00-constitutional/  ← MVG, PLATFORM_ARCHITECTURE
├── 01-authoritative/   ← AUTHORITATIVE_RUNTIME_MAP, PHASE_GATES, API_CONTRACTS
├── 02-master/          ← MASTER_*_STRATEGY docs
├── 03-engines/         ← ENGINE_*_STRATEGY docs
├── 04-vision/          ← Future direction docs
└── 05-governance/      ← DOCUMENT_REGISTRY, DOC_ID_REGISTRY
```
...
## 15. Link Validation Process

### 15.1 When to Validate Links

| Trigger | Scope | Who |
|---------|-------|-----|
| Creating index file (00_MASTER_INDEX, README) | All links in file | Author |
| Creating strategy doc with references | All links in file | Author |
| /close command | Changed files | Automated |
| /audit command | All index files | Automated |
| Post-folder reorganization | All affected paths | Manual audit |

### 15.2 Link Validation Checklist

```
FOR EACH MARKDOWN LINK [text](path):

1. PATH TYPE CHECK:
   [ ] Relative path (./path) → Verify from current file location
   [ ] Root path (/path) → Verify from project root
   [ ] Parent path (../path) → Count parent levels correctly

2. FILE EXISTS CHECK:
   [ ] Run: ls <resolved_path> OR Glob tool
   [ ] If NOT FOUND → Flag as broken

3. PATH STRUCTURE CHECK:
   [ ] Check for missing subfolder in path
   [ ] Common issue: strategy/ files moved to strategy/0X-folder/

4. RESOLUTION:
   [ ] Fix path if file exists elsewhere
   [ ] Mark as [TODO] if file planned but not created
   [ ] Remove link if file deprecated/deleted
```

### 15.3 Automated Link Audit (for /audit command)
...
1. Extract all markdown links from:
   - IBRAIN_STRATEGY/00_MASTER_INDEX.md
   - All folder README.md files
   - All documents modified this session
...
3. Report:
   - Total links checked
   - Broken links (with file:line reference)
   - Suggested fixes
```

### 15.4 Common Broken Link Patterns

| Pattern | Cause | Fix |
|---------|-------|-----|
| `/strategy/FILE.md` | File moved to subfolder | Add subfolder: `/strategy/03-engines/FILE.md` |
| `/1-PRODUCT/FILE.md` | File never created | Create file or mark [TODO] |
| `../../../path` | Wrong parent count | Recount directory levels |
| `./path` from wrong location | Copy-paste error | Adjust relative path |

### 15.5 Prevention Best Practices

1. **Use autocomplete**: When possible, use IDE/tool autocomplete for paths
2. **Copy existing links**: Copy links from existing documents that work
3. **Verify after creation**: Always test links after creating index files
4. **Update when moving files**: When reorganizing folders, update ALL referencing docs
5. **Mark planned files**: Use `[PLANNED]` or `[TODO]` suffix for files not yet created
