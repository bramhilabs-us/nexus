...
### IMP-SPRINT1-001: Password Reset Flow via Email Link
- **Priority**: P2 (NICE TO HAVE)
- **Phase**: Sprint 2 or Beta Q1 2026
- **Effort**: 6 hours
- **Discovered**: Sprint 1 Planning (Nov 5, 2025)
- **Description**: Replace temp password in email with password reset link
- **Current**: Sprint 1 sends temp password in plain text email (security concern)
- **Proposed**: Send magic link that expires in 24 hours, user sets password via link
- **Business Value**: Better security, standard auth pattern
- **Files to Create**:
  - client/pages/password-reset.html (password reset form)
  - POST /api/auth/password-reset/:token (backend endpoint)
- **Files to Modify**:
  - server/services/mailjetService.js (update company invitation template)
  - server/routes/invitations.js (generate reset token instead of temp password)
- **Status**: Planned for Sprint 2
- **Related**: Sprint 1 temporary solution uses temp password for faster MVP
